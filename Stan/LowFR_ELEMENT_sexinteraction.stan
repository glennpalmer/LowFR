// Final model for fitting to ELEMENT data with added interaction terms
// for all exposures with sex.
...
parameters {
  ...
  // interactions for exposures with sex
  vector[k] beta_sex;
  vector[TT] omega_sex;
  ...
  // parameters for multiplicative gamma process prior -- interactions with sex
  real<lower=0> tau_sex;
  real<lower=0> xi_sex[k+TT];
  real<lower=0> a1_sex;
...
transformed parameters {
  ...
  // declare theta_sex for interactions with sex
  vector[k*TT] theta_sex;
  ...
  // define theta_sex in terms of beta_sex and omega_sex
  for (j in 1:k) {
    for (t in 1:TT) {
      theta_sex[t + (j-1) * TT] = beta_sex[j] * omega_sex[t];
    }
  }
...
model {
  ...
  // beta_sex and omega_sex multiplicative gamma priors
  for (j in 1:k) {
    beta_sex[j] ~ normal(0, 1 / sqrt(xi_sex[j] * tau_sex));
  }
  
  for (t in 1:TT) {
    omega_sex[t] ~ normal(0, 1 / sqrt(xi_sex[k + t] * tau_sex));
  }
  ...
  // priors for multiplicative gamma hyperparameters -- sex interactions
  tau_sex ~ gamma(a1_sex, 1);
  xi_sex ~ gamma(1.5, 1.5);
  a1_sex ~ gamma(2,1);
  ...
  for (i in 1:N) {
    ...
    y[i] ~ normal(mu + Eta[i] * theta + quad_form(Omega, Eta[i,]') +
                  ...
                  alpha_bmi * bmi[i] + alpha_onset * onset[i] +
                  Eta[i] * theta_sex * sex[i], sqrt(sigma2));
  }
}
