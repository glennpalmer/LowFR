...
// coefficients.
// 
// This version is also set up to run cross-validation via an input training
// and test set.
//
...
data {
  ...
  matrix[N_train, r] Z_train;
  vector[N_test] y_test;
  matrix[N_test, p*TT] X_test;
  matrix[N_test, r] Z_test;
}
...
model {
  ...
  // likelihood
  for (i in 1:N_train) {
    y_train[i] ~ normal(alpha_0 + X_train[i]*alpha + quad_form(Gamma, X_train[i,]') + Z_train[i]*beta, sigma2);
  }
}
...
generated quantities {
  ...
  for (i in 1:N_test) {
    y_test_pred[i] = alpha_0 + X_test[i]*alpha + quad_form(Gamma, X_test[i,]') + Z_test[i]*beta;
  }
